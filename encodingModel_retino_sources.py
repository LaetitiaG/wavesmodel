    ...
    for ses in sessions:
        ...
        if plot_stc:
            ...
            for ind_c, cond in enumerate(cond_waves): 
                brain = stc_waves[ind_c].plot(subject=subject, subjects_dir=subjects_dir, views = 'caudal', 
                                     clim = {'kind': 'value', 'pos_lims': [0, A/2,A]}, colormap = 'mne', smoothing_steps = 2)
        ...
        if ses=='session1':
            for ind_c, cond in enumerate(cond_waves): 
                ...
                # Save stc
                fname = op.join(simupath,'{}_simulated_source_leftV1_stimulated{}_otherHalf{}'.format(subject, cond, cond_waves[not(ind_c)]))
                tmp.save(fname)  
        ...
        speed_label = [];  global_speed = [];  global_speed_label = []
        for ind_h, hemi in enumerate(hemis):
            # Compute real cortical speed
            vert_coord = src[ind_h]['rr'][inds_label[ind_h]] # coordinates of each vertex of the label Freesurfer surface RAS in m
            vert_dis = scipy.spatial.distance.cdist(vert_coord, vert_coord,metric='euclidean')*1000 # distance for each pair of vertices in mm
            ...
            for a in np.arange(0,181):
                ...
                for i in range(len(ind_horiz)-1):
                    ...
                    s1 = wave_label[ind_h][0,ind_horiz[i]]; s2 =wave_label[ind_h][0,ind_horiz[i+1]]
                ...
                i1 = 0; i2 = -1 
                s1 = wave_label[ind_h][0,ind_horiz[i1]]; s2 =wave_label[ind_h][0,ind_horiz[i2]]
                while np.sum(s1) ==0: # select closest voxel near the fovea with non zero signal
                    i1 = i1+1; s1 = wave_label[ind_h][0,ind_horiz[i1]];
                while np.sum(s2) ==0: # select closest voxel near the periph with non zero signal
                    i2 = i2-1; s2 =wave_label[ind_h][0,ind_horiz[i2]]
                ...
                i1_list.append(i1); i2_list.append(i2)
            ...
            print('Range speed  = [{:.3f}, {:.3f}] cycles/mm'.format(np.nanmin(computed_speed), np.nanmax(computed_speed)))
            print('Global mean speed  = {:.3f} cycles/mm and median = {:.3f}'.format(np.mean(global_speed), np.median(global_speed)))
